...
int main() try
{
    ...
    while (running)
    {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            glClearColor(glClearColors[i][0], glClearColors[i][1], glClearColors[i][2], glClearColors[i][3]);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            ...
            if (i != 0) {
                projection = glm::ortho(-(float)width/height,(float)width/height,-1.f,1.f);
            }
